...
const STATEMENT_IGNORE_PATTERN =
  /(total spent|payment summary|generated by|share summary|grand total|total amount)/i;
const MONTH_DATE_PATTERN =
  /\b((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}(?:,\s*\d{4})?)\b/i;
const STATEMENT_ROW_PATTERN =
  /\b((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}(?:,\s*\d{4})?)\b\s+(.+?)\s+([A-Za-z]+)\s+(₹?\d{1,3}(?:,\d{3})*(?:\.\d{2})?|₹?\d+(?:\.\d{2})?)\b/i;
...
const statementTransactionsFromOCR = (processed: OcrPreprocessResult): ExtractedReceiptTransaction[] => {
  ...
  for (const line of lines) {
    ...
    if (/^date\s+description\s+category\s+amount$/i.test(line)) continue;

    const match = line.match(STATEMENT_ROW_PATTERN);
    if (!match) continue;

    const [, rawDate, rawDescription, rawCategory, rawAmount] = match;
    ...
    const description = rawDescription.replace(MONTH_DATE_PATTERN, '').trim();
    if (!description || STATEMENT_IGNORE_PATTERN.test(description)) continue;

    extracted.push({
      date,
      merchant: description,
      amount,
      category: mapCategory(rawCategory, description),
      confidence: 0.95,
    });
  }
...
export async function analyzeReceiptLocally(
  ...
  const isStatementMode = STATEMENT_TRIGGER_PATTERN.test(processed.normalizedLower);
  if (isStatementMode) {
    const statementTransactions = statementTransactionsFromOCR(processed);
    devLog('Statement parser transactions', statementTransactions);
    if (statementTransactions.length >= 2) {
      return {
  ...
  if (!options.llmReady) {
    console.warn('[SpendSense][Receipt] AI model not ready, using regex-only parser', {
      modelStatus: options.modelStatus ?? 'unknown',
    });
    const fallbackTransactions = fallbackTransactionsFromOCR(processed);
    if (fallbackTransactions.length === 0) {
      throw new Error('AI model not ready. Please wait.');
    }
    return {
  ...
  onStageChange?.('llm');
  console.info('[SpendSense][Receipt] LLM call attempt', {
    modelStatus: options.modelStatus ?? 'ready',
  });
  ...
  try {
    ...
    console.error('[SpendSense][Receipt] LLM extraction failed, falling back to regex parser', error);
  }
