    ...
    <section className="space-y-4">
      <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <label
          ...
          className="mt-4 block cursor-pointer rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center transition hover:border-sky-400 hover:bg-sky-50"
        >
          <input
            type="file"
            accept="image/*,.pdf"
          ...
          <p className="mt-1 text-xs text-slate-500">or tap to choose a file</p>
        </label>
      ...
      {results.length > 0 && (
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <h3 className="text-base font-semibold text-slate-900">Extracted Transactions (Mock)</h3>
          <ul className="mt-3 space-y-2">
            {results.map((row, index) => (
              ...
              >
                <div>
                  <p className="font-medium text-slate-800">{row.merchant}</p>
